...
public class Allocation extends AbstractPersistable implements Comparable<Allocation> {
    ...
    public int compareTo(@NotNull Allocation o) {
        return Comparator.comparing(Allocation::getConstrainedStartDate, Comparator.nullsLast(Comparator.naturalOrder()))
    ...
    private long getTimeRestrictionScore(String restriction) {
        Range<ZonedDateTime> thisRange = Range.closed(getConstrainedStartDate(), getEndDate());
    ...
    // Complex methods
    // ************************************************************************
    public ZonedDateTime getConstrainedStartDate() {
        // Built-in hard constraint for delay, applies only to those affecting scores
        // (planning variables, listeners, score, resulting schedule)
        if (timelineEntry.getChronoProperty().getDraggable() == 0 &&
                timelineEntry.getChronoProperty().getZonedStartTime() != null) {
    ...
    public ZonedDateTime getEndDate() {
        try{
            return getConstrainedStartDate().plus(plannedDuration);
        }catch (Exception ex){
            return null;
        }
    ...
    public int getConstrainedProgressDelta() {
        // Built-in hard constraint for progressdelta, applies only to those affecting scores
        // (planning variables, listeners, score, resulting schedule)
        return getTimelineEntry().getChronoProperty().getSplittable() == 0 ?
                getTimelineEntry().getProgressChange().getProgressDelta100() :
                getProgressdelta();
    }
