...
public class TCGuiController {
    ...
    private void initialize() {
        Platform.runLater(() -> {
            ...
            percentColumn.setCellFactory(x -> new TableCell<>() {
                ...
                protected void updateItem(Integer item, boolean empty) {
                    ...
                    if (empty || thisObject == null) {
                        ...
                        Integer originalValue = thisObject.getConstrainedProgressDelta();
            ...
            percentColumn.setCellValueFactory(x -> new SimpleObjectProperty<>(x.getValue().getConstrainedProgressDelta()));
            ...
            startdateColumn.setCellFactory(col -> new TableCell<>() {
                ...
                public void updateItem(ZonedDateTime item, boolean empty) {
                    super.updateItem(item, empty);
                    Allocation thisObject = this.getTableRow().getItem();
                    if (empty || thisObject == null || thisObject.getConstrainedStartDate() == null) {
                        setText(null);
                    } else {
                        LocalDateTime originalValue = item.withZoneSameInstant(defaultTimeZone).toLocalDateTime();
            ...
            startdateColumn.setCellValueFactory(x -> new SimpleObjectProperty<>(x.getValue().getConstrainedStartDate()));
                        ...
                        if (change.getElementAdded() != null) {
                            ...
                            Platform.runLater(() -> {
                                ...
                                ((ZonedDateTimeAxis) resourceLineChart.getXAxis())
                                        .setLowerBound(selected.getUserObject().getConstrainedStartDate().minusDays(7));
                                ((ZonedDateTimeAxis) resourceLineChart.getXAxis())
        ...
        Platform.runLater(() -> {
            ...
            allocationCalendarEntryMap.forEach((key, value) -> {
                if (!displayingAllocations.contains(key) || !key.isFocused()) {
                    ...
                    value.setInterval(
                            key.getConstrainedStartDate().withZoneSameInstant(defaultTimeZone),
                            key.getEndDate().withZoneSameInstant(defaultTimeZone));
                    ...
                    ChangeListener<Interval> intervalChangeListener = (observable, oldValue, newValue) -> {
                        setAllocationVal(key, "progressdelta", (int) Math.min(100,
                                (key.getTimelineEntry().getProgressChange().getProgressDelta100() *
    ...
    private void setAllocationVal(Allocation thisObject, String propertyName, Object newValue) {
        ...
        if (propertyName.equals("start_date") && newValue instanceof LocalDateTime) {
            ...
            pendingAllocationValues.get(thisObject).setPlanningStartDate(
                    ((LocalDateTime) newValue).atZone(thisObject.getConstrainedStartDate().getZone()));
        }
